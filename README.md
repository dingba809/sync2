...
同步以本地目录为准。首次同步会安全合并目标目录：内容相同的同路径文件会被认领而不重新上传，远端独有文件会保留；同路径内容不同则以本地版本替换。后续同步仅比较本地文件与 SQLite 快照：大小和修改时间一致时直接跳过；只有元数据变化时才计算 MD5/SHA1 作进一步确认。删除本地文件只会删除此前由该任务管理过的远端文件。

日志页的「文件审计明细」可按任务和单次运行查看每个文件的结果（跳过、认领、上传、替换、删除、失败或冲突）。明细仅写入本地 SQLite，不会增加网盘请求；系统按任务保留最近 90 天且最多 30 次运行的明细，并使用分页加载避免大目录占用浏览器内存。

选择包含上传或删除失败项的运行后，可点击「重试失败文件」。系统会创建新的运行批次，并且只重试该批次记录的失败路径；成功、跳过和冲突项不会重复执行。
