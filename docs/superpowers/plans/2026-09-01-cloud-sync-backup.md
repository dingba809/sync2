...
export class QuarkProvider implements DriveProvider {
  ...
  async uploadFile(localPath: string, parentId: string, name: string): Promise<RemoteEntry> {
    ...
    if (hashRes?.fid) {
      return { id: hashRes.fid, name, isDir: false, size, mtime: Math.floor(Date.now() / 1000), hash: md5 };
    }

    const auth = await this.post(`${BASE}/file/upload/auth`, {
      task_id: pre.task_id, auth_info: pre.auth_info, auth_meta: ''
    });
    const uploadUrl = `${pre.upload_url || `https://${pre.bucket}.pds.quark.cn/${pre.obj_key}`}`;
    const buf = readFileSync(localPath);

    if (size < 5 * 1024 * 1024) {
      await this.putPart(uploadUrl, 1, pre.upload_id, buf, auth.auth_key);
    } else {
      const chunkSize = 4 * 1024 * 1024;
      let part = 1;
      for (let off = 0; off < size; off += chunkSize) {
        await this.putPart(uploadUrl, part++, pre.upload_id, buf.subarray(off, off + chunkSize), auth.auth_key);
      }
    }
  ...
  private async putPart(uploadUrl: string, partNumber: number, uploadId: string, body: Buffer, authKey: string): Promise<void> {
    const url = `${uploadUrl}?partNumber=${partNumber}&uploadId=${uploadId}`;
    ...
    if (!res.ok) throw new Error(`Quark part upload failed: ${res.status}`);
  }
